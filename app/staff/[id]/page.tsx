      ...
      <div className="mx-auto max-w-7xl px-8 py-6 sm:py-10">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-7">
          ...
          <div className="min-w-0">
            ...
            {staff.name.native && (
              <p className="text-lg text-gray-400 mt-1">{staff.name.native}</p>
            )}
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {staff.occupations.slice(0, 3).map((occ) => (
                <span
                  key={occ}
                  className="rounded-full bg-white/[0.06] border border-white/10 px-3 py-1 text-xs text-gray-300 capitalize"
                >
                  {occ}
                </span>
              ))}
        ...
        <section className="mt-10">
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6">
            Voice Roles{' '}
            <span className="text-sm font-normal text-gray-500">({staff.works.length})</span>
          </h2>

          {staff.works.length === 0 ? (
            <p className="text-gray-500 text-sm">No anime voice roles found.</p>
          ) : (
