...
export function JapandiBrowseGridSkeleton() {
  return (
    <div className="space-y-5">
      <div
        className="flex items-start gap-3 rounded-xl border border-line/10 bg-surface-2/40 px-4 py-3"
        role="status"
        aria-live="polite"
        aria-busy="true"
      >
        ...
        <div>
          <p className="text-sm font-medium text-fg-muted">Loading this season&apos;s catalog…</p>
...
export async function JapandiBrowseAnimeGrid({
  ...
  try {
    const animeList = await fetchAnime({ season, year, genres, formats, studio });

    if (!animeList || animeList.length === 0) {
      const filtered =
        (genres && genres.length > 0) || (formats && formats.length > 0) || !!studio;
      return (
