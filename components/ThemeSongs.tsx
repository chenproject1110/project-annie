...
export async function ThemeSongs({ idMal }: { idMal: number | null }) {
  ...
  return (
    <div>
      <h2 className="text-xl sm:text-2xl font-bold text-fg mb-3 sm:mb-4">Theme Songs</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        {themes.openings.length > 0 && (
          <div className="p-4 sm:p-5 bg-surface rounded-lg">
            ...
            <ul className="space-y-2.5">
              {themes.openings.map((op) => (
                <li key={op.slug} className="text-sm leading-relaxed">
                  ...
                  <span className="text-fg">&ldquo;{op.songTitle}&rdquo;</span>
                  {op.artists.length > 0 && (
                    <span className="text-fg-muted"> by {op.artists.join(', ')}</span>
                  )}
                  {op.episodes && (
                    <span className="text-fg-muted text-xs ml-1.5">(ep. {op.episodes})</span>
                  )}
                </li>
              ))}
            </ul>
            ...
            <ul className="space-y-2.5">
              {themes.endings.map((ed) => (
                <li key={ed.slug} className="text-sm leading-relaxed">
                  ...
                  <span className="text-fg">&ldquo;{ed.songTitle}&rdquo;</span>
                  {ed.artists.length > 0 && (
                    <span className="text-fg-muted"> by {ed.artists.join(', ')}</span>
                  )}
                  {ed.episodes && (
                    <span className="text-fg-muted text-xs ml-1.5">(ep. {ed.episodes})</span>
                  )}
                </li>
              ))}
            </ul>
...
export function ThemeSongsSkeleton() {
  return (
    <div>
      <div className="h-7 sm:h-8 w-40 rounded-lg bg-line/10 animate-pulse mb-3 sm:mb-4" />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <div className="h-32 rounded-lg bg-line/5 animate-pulse" />
        <div className="h-32 rounded-lg bg-line/5 animate-pulse" />
      </div>
    </div>
  );
